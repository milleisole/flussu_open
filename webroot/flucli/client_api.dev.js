...
function extractCalendar(elms) {
    ...
    if (Object.keys(dateToSlots).length) {
        ...
        Object.keys(dateToSlots).sort().forEach(date => {
            html += `<div class="bg-gray-100 dark:bg-gray-800 rounded-2xl p-4 min-w-[150px] shadow flex flex-col items-center">`;
            // Formatta data in modo leggibile
            let dateObj = new Date(date.replace(/\//g, "-"));
            html += `<div class="font-bold mb-2 text-blue-900 dark:text-blue-200">${dateObj.toLocaleDateString("it-IT", { weekday: "short", year: "2-digit", month: "short", day: "numeric" })}</div>`;
            // Bottoni slot
