...
class FlussuZeroOneAi implements IAiProvider
{
    ...
    private $_zeroone_ai;
    private $_zeroone_ai_key="";
    private $_zeroone_ai_model="";
    private $_zeroone_chat_model="";
    private $client;
    ...
    public function __construct($model="", $chat_model=""){
        if (!isset($this->_zeroone_ai)){
            ...
            if ($model)
                ...
                if (!empty(config("services.ai_provider.zeroone_ai.model")))
                    $this->_zeroone_ai_model=config("services.ai_provider.zeroone_ai.model");
            }
            if ($chat_model)
                $this->_zeroone_chat_model = $chat_model;
            else {
                if (!empty(config("services.ai_provider.zeroone_ai.chat-model")))
                    $this->_zeroone_chat_model=config("services.ai_provider.zeroone_ai.chat-model");
            }
            $this->client = new Client([
                'base_uri' => 'https://api.01.ai/v1/',
                'timeout'  => 10.0,
            ]);
        }
    }
    ...
    private function _chatContinue($arrayText){
        ...
        try {
            $response = $this->client->post('chat/completions', [
                'headers' => [
                    'Authorization' => 'Bearer ' . $this->_zeroone_ai_key,
            ...
            if (isset($data['choices'][0]['message']['content']))
                ...
                return [$arrayText, "Error: no 01.AI response. Details: " . print_r($data, true), null];
    ...
    public function generateImage($prompt, $size="1024x1024", $quality="standard"): array {
        return ["error" => "Image generation not supported by 01.AI"];
    }
